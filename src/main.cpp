...
#include <math.h>     //libreria para usar la funcion pow()
...
int RecValue=0;     //valor para convertir el texto recivido en un valor

WiFiClient espClient;   //creamos una instancia de WifiClient
PubSubClient client(espClient);   //Indicamos que usamos dicha instancia para MQTT
long lastMsg = 0;   //variable para el conteo de mensajes
char msg[50];       //variable para guardar el mensaje que se envia.
int value = 0;      ///////////////////////////////////////////////////////

void setup_wifi();    //se declara la rutina setup_wifi
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    // Attempt to connect
    if (client.connect("TibuESP8266Client2")) {
      Serial.println("connected");
      // Once connected, publish an announcement...
      client.publish("TibuOutTopic", "hello world");
      // ... and resubscribe
      client.subscribe("TibuOutTopic");
    } else {
...
/*  if (now - lastMsg > 2000) {
    ...
    Serial.println(msg);
    client.publish("TibuInTopic", msg);
  }
*/
}
